...
namespace KingdomsRebellion.Core {
	public static class KRFacade {
		...
		public static void Add(KRTransform t, bool floating = true) {
			_Map.Add(t, floating);
		}

		public static void Remove(KRTransform t, bool floating = true) {
			_Map.Remove(t, floating);
		}
		...
		public static bool IsInBounds(Vec2 v) {
			return _Map.IsInBounds(v);
		}
		...
		public static IEnumerable<GameObject> Find(Vec2 v1, Vec2 v2, Vec2 v3) {
			...
			Func<Vec2,bool> _in = delegate(Vec2 v) {
				...
				if ((x - ax) * ex + (y - ay) * ey < 0)
					return false;
				if ((x - bx) * ex + (y - by) * ey > 0)
					return false;
				if ((x - ax) * fx + (y - ay) * fy < 0)
					return false;
				if ((x - dx) * fx + (y - dy) * fy > 0)
					return false;
...
//			Debug.Log(String.Format("DragFind :: time elapsed: {0}", stopwatch.Elapsed));
		...
		public static Vec2 NearSquareAt(Vec2 v1, Vec2 v2, int dist) {
			...
			while (!_Map.IsEmpty(left) || !_Map.IsEmpty(right)) {
				...
				if (left == leftT)
					onTop = true;
				if (left == right && !_Map.IsEmpty(left))
					return null;
			}
