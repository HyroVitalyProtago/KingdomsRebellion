...
namespace KingdomsRebellion.Core.Player {
    ...
    public class Attack : KRBehaviour {
        ...
        private void OnAttack(int playerID, Vec3 modelPoint) {
            Target = _grid.GetGameObjectByPosition(new Vec2(modelPoint.X, modelPoint.Z));
        }

        // TODO remove and replace _isAttacking by _unit.attacking
        public void UpdateGame() {
            Vec2 targetPos = _grid.GetPositionOf(Target);
            if (Vec2.Dist(targetPos, _grid.GetPositionOf(gameObject)) == 1) {
                if (attackSpeed == 0) {
                    spot.SetActive(true);
