    ...
    public class AttackState : FSMState {
        ...
        public override void Execute() {
            if (_attack.Target != null &&
                Vec2.Dist(_grid.GetPositionOf(_attack.Target), Vec2.FromVector3(fsm.gameObject.transform.position)) ==
                _attack.range) {
                Debug.Log("FRAPPE !");
                _attack.UpdateGame();
            } else if (_attack.Target == null) {
                List<GameObject> gameObjects = _grid.GetNearGameObjects(Vec2.FromVector3(fsm.gameObject.transform.position), 6);
